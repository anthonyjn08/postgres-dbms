...
# artist = session.query(Artist).filter_by(Name="Marvin Gaye").first()
# print(artist.ArtistId, artist.Name, sep=" | ")

# Query 4 - select only by the "ArtistId" #104 from the "Artist" table
# artist = session.query(Artist).filter_by(ArtistId=104).first()
# print(artist.ArtistId, artist.Name, sep=" | ")

# Query 5 - select on the albums with "ArtistId" #104 on the "Album" table
# albums = session.query(Album).filter_by(ArtistId=104)
# for album in albums:
#     print(album.AlbumId, album.Title, album.ArtistId, sep="  |  ")

# Query 6 - select all tracks where the composer is "Marvin Gaye" from the "Track" table
tracks = session.query(Track).filter_by(Composer="Marvin Gaye")
for track in tracks:
    print(track.TrackId,
    track.Name,
    track.AlbumId,
